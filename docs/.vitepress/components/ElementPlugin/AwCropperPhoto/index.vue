...
import { computed, PropType, ref, watch, defineAsyncComponent } from "vue";
...
const VueCropper = computed(() => {
  const vueCropper = defineAsyncComponent(
    () => import("vue-cropper/lib/vue-cropper.vue")
  );
  return vueCropper;
}) as ComputedRef;
// console.log(Cropper.value, 8885);
// const { VueCropper } = Cropper.value;
...
const cropperRef = ref<InstanceType<typeof VueCropper.value>>();
const option = ref(
  Object.assign(
    {
      img: "", //裁剪图片的地址
